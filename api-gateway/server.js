...
app.use(async (req, res, next) => {
  ...
  if (publicRoutes.includes(req.path)) {
    return next(); // Lewati cek untuk rute publik
  }
  ...
  jwt.verify(token, PUBLIC_KEY, { algorithms: ['RS256'] }, (err, decoded) => {
    ...
    // Token valid! Suntikkan data user ke header request
...
// Proxy configurations (tetap sama)
...
// Catch-all route dan Error handling
